...
#include <SSL/mbedtls/asn1write.h>
 ...
 */
static int pk_write_sm2_pubkey( unsigned char **p, unsigned char *start,
                                 sm2_context *ec )
{
    int ret;
    size_t len = 0;
    ...
    if( ( ret = mbedtls_ecp_point_write_binary( &ec->grp, &ec->Pb,
                                        MBEDTLS_ECP_PF_UNCOMPRESSED,
                                        &len, buf, sizeof( buf ) ) ) != 0 )
    {
        return( ret );
...
#endif
#if defined(MBEDTLS_SM2_C)
    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_SM2 )
        MBEDTLS_ASN1_CHK_ADD( len, pk_write_sm2_pubkey( p, start, mbedtls_pk_sm2( *key ) ) );
    else
#endif
...
int mbedtls_pk_write_key_der( mbedtls_pk_context *key, unsigned char *buf, size_t size )
{
    ...
    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_SM2 )
    {
        sm2_context *sm2 = mbedtls_pk_sm2( *key );
        size_t pub_len  = 0;
        ...
        /* publicKey */
        MBEDTLS_ASN1_CHK_ADD( pub_len, pk_write_sm2_pubkey( &c, buf, sm2 ) );
        ...
        *--c = 0;
        ...
        /* privateKey: write as MPI then fix tag */
        MBEDTLS_ASN1_CHK_ADD( len, mbedtls_asn1_write_mpi( &c, buf, &sm2->d ) );
        *c = MBEDTLS_ASN1_OCTET_STRING;
...
#define PEM_BEGIN_PRIVATE_KEY_SM2   "-----BEGIN SM2 PRIVATE KEY-----\n"
#define PEM_END_PRIVATE_KEY_SM2     "-----END SM2 PRIVATE KEY-----\n"
...
#if defined(MBEDTLS_ECP_C) || defined(MBEDTLS_SM2_C)
/*
 * EC public keys:
...
int mbedtls_pk_write_key_pem( mbedtls_pk_context *key, unsigned char *buf, size_t size )
{
    ...
    if( mbedtls_pk_get_type( key ) == MBEDTLS_PK_SM2 )
    {
        begin = PEM_BEGIN_PRIVATE_KEY_SM2;
        end = PEM_END_PRIVATE_KEY_SM2;
    }
    else
#endif
        return( MBEDTLS_ERR_PK_FEATURE_UNAVAILABLE );
