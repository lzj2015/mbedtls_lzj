...
#include <SSL/mbedtls/pk_internal.h>
...
#include <SSL/mbedtls/platform.h>
#else
#include <stdlib.h>
...
static size_t sm2_get_bitlen( const void *ctx )
{
    return( 8 * ((const sm2_context *) ctx)->grp.pbits );
}
                   ...
                   const unsigned char *sig, size_t sig_len )
{
    ...
    if( md_alg == MBEDTLS_MD_NONE && mbedtls_md_get_size(md_info) > 32 )
        return( MBEDTLS_ERR_PK_INVALID_ALG  );
    if( mbedtls_md_get_size(md_info) < hash_len )
        return( MBEDTLS_ERR_PK_BAD_INPUT_DATA );
    return sm2_do_verify((sm2_context *)ctx, hash, hash_len,
		sig, sig_len);
}
                   ...
                   int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    ...
    // TODO: Danger! sign_max_len must be more than 64! 
    return sm2_do_sign((sm2_context *)ctx, hash, hash_len,
		sig, 64, sig_len, f_rng, p_rng);
}
...
{
    if(f_rng || p_rng)
         return sm2_do_decrypt((sm2_context *)ctx, input, ilen,
                   output, osize, olen);
     return sm2_do_decrypt((sm2_context *)ctx, input, ilen,
                   output, osize, olen);
}
...
static int sm2_encrypt_wrap( void *ctx,
                    ...
                    int (*f_rng)(void *, unsigned char *, size_t), void *p_rng )
{
    return sm2_do_encrypt((sm2_context *)ctx, input, ilen,
                   output, osize, olen,
                   f_rng, p_rng);
}
...
{
    void *ctx = mbedtls_calloc( 1, sizeof( sm2_context ) );

    if( ctx != NULL )
        sm2_init( (sm2_context *) ctx );
...
{
    sm2_free( (sm2_context *) ctx );
    mbedtls_free( ctx );
}

static void sm2_debug( const void *ctx, mbedtls_pk_debug_item *items )
{
    items->type = MBEDTLS_PK_DEBUG_MPI;
    items->name = "sm2.D";
    items->value = &( ((sm2_context *) ctx)->d );

    items++;

    items->type = MBEDTLS_PK_DEBUG_MPI;
    items->name = "sm2.pb.X";
    items->value = &( ((sm2_context *) ctx)->Pb.X );
    items++;

    items->type = MBEDTLS_PK_DEBUG_MPI;
    items->name = "sm2.pb.Y";
    items->value = &( ((sm2_context *) ctx)->Pb.Y );
    items++;

    items->type = MBEDTLS_PK_DEBUG_MPI;
    items->name = "sm2.pb.Z";
    items->value = &( ((sm2_context *) ctx)->Pb.Z );
}
