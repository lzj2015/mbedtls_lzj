...
#if defined(MBEDTLS_SM2_C)
/*
 * SM2 public key is an SM2 point
 *
 * The caller is responsible for clearing the structure upon failure if
 * desired. Take care to pass along the possible ECP_FEATURE_UNAVAILABLE
 * return code of mbedtls_ecp_point_read_binary() and leave p in a usable state.
 */
static int pk_get_sm2pubkey( unsigned char **p, const unsigned char *end,
                            sm2_context *key )
{
    int ret;

    if( ( ret = mbedtls_ecp_point_read_binary( &key->grp, &key->Pb,
                    (const unsigned char *) *p, end - *p ) ) == 0 )
    {
        ret = mbedtls_ecp_check_pubkey( &key->grp, &key->Pb );
    }
                        ...
                        mbedtls_pk_context *pk )
{
    ...
    {
        ret = pk_get_sm2pubkey( p, end, mbedtls_pk_sm2( *pk ) );
    } else
#endif /* MBEDTLS_SM2_C */
...
#if defined(MBEDTLS_SM2_C)
/*
 * Parse a encoded private SM2 key
 */
static int pk_parse_key_sm2_der( sm2_context *sm2,
                                  const unsigned char *key,
                                  size_t keylen )
{
    int ret;
    ...
    if( ( ret = mbedtls_mpi_read_binary( &sm2->d, p, len ) ) != 0 )
    {
        sm2_free( sm2 );
        return( MBEDTLS_ERR_PK_KEY_INVALID_FORMAT + ret );
    }
    ...
    if( p != end )
    {
                        ...
                        MBEDTLS_ASN1_CONTEXT_SPECIFIC | MBEDTLS_ASN1_CONSTRUCTED | 1 ) ) == 0 )
        {
            ...
            if( ( ret = pk_get_sm2pubkey( &p, end2, sm2 ) ) == 0 )
                pubkey_done = 1;
            else
            {
                /*
                 * The only acceptable failure mode of pk_get_sm2pubkey() above
                 * is if the point format is not recognized.
                 */
                if( ret != MBEDTLS_ERR_ECP_FEATURE_UNAVAILABLE )
                    return( MBEDTLS_ERR_PK_KEY_INVALID_FORMAT );
    ...
    if( ! pubkey_done &&
        ( ret = mbedtls_ecp_mul( &sm2->grp, &sm2->Pb, &sm2->d, &sm2->grp.G,
                                                      NULL, NULL ) ) != 0 )
    {
        sm2_free( sm2 );
        return( MBEDTLS_ERR_PK_KEY_INVALID_FORMAT + ret );
    ...
    if( ( ret = sm2_check_public( sm2 ) ) != 0 )
    {
        sm2_free( sm2 );
        return( ret );
    }

    if( ( ret = sm2_check_private( sm2 ) ) != 0 )
    {
        sm2_free( sm2 );
        return( ret );
    }
                  ...
                  const unsigned char *pwd, size_t pwdlen )
{
    ...
    if( ret == 0 )
    {
        ...
        if( ( ret = mbedtls_pk_setup( pk, pk_info                   ) ) != 0 ||
            ( ret = pk_parse_key_sm2_der( mbedtls_pk_sm2( *pk ),
                                           pem.buf, pem.buflen ) ) != 0 )
        {
            mbedtls_pk_free( pk );
        }
    ...
    if( ( ret = mbedtls_pk_setup( pk, pk_info                         ) ) != 0 ||
        ( ret = pk_parse_key_sm2_der( mbedtls_pk_sm2( *pk ), key, keylen ) ) == 0 )
    {
        return( 0 );
    }
